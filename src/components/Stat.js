  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col items-center justify-center pt-3 text-white">
        <span className={` uppercase ${gaugeColor}  ${size === 'md' ? 'text-xxs' : 'text-xxxs'}`}>
          {stats.title}
        </span>
        <span className={`leading-tight ${gaugeColor}  ${size === 'md' ? 'text-3xl' : 'text-2xl'}`}>
          {stats.value}
        </span>
        <span className={`leading-normal ${gaugeColor}  ${size === 'md' ? 'text-xl' : 'text-base'}`}>
          {stats.unit}
        </span>
      </div>
    </div>
